 ...
 * */
var lpage = {
    checkBrandName: function(){
        var paramBrandName = $('#paramBrandName').val();
        if(paramBrandName != '' && paramBrandName != null && paramBrandName != undefined){
            paramBrandName = paramBrandName.trim();
            switch(paramBrandName){
                case 'dodge':
                    makeName = 'dodge';
                    break;
                case 'fiat':
    ...
    init: function() {
        var brandName = lpage.checkBrandName();
        if (localCache.exist('oremake')) {
            var makeName = localCache.get('oremake');
            if(makeName != '' && makeName != null && makeName != undefined){
                if(makeName != brandName){
                    localCache.set('oremake', brandName);
                }
            }
        ...
        $(document).on('click', '#selectedZipCodePopupBtn', function() {
         ...
         $(document).on('click', '.ZipCodeDivCls', function() {
            $('.zipcodePopup_errorlocationInfo span,.zipcodePopup_errorInfo span').text('');
            var zipcode =$('#selectedZipCode').val();            
            zipcode = AddZeroPrefixZipcode(zipcode);
            if ( zipcode !== '') {
    ...
    landingRequest: function() {
        ...
        if (!localCache.exist('zipCodeEntered')) {
            if(($("#zipCodePopup").data('bs.modal') || {isShown: false}).isShown){
                $('.zipcodePopup_errorlocationInfo span,.zipcodePopup_errorInfo span').text('');
    ...
    NewVehicleLoader: function(make, vechType, zipcode) {
        ...
        ajax.promise('ore/' + make  + '/' + zipcode, 'get', JSON.stringify({}));
    },
    /***
        ...
        switch (url) {
            ...
            case "validate_zipcode": 
                ...
                if (result['status'] == 'available') {
                    if(!localCache.get('zipCodeEntered') || localCache.get('zipCodeEntered')==''){
                      landingtrue = 'yes';
                    }
                   ...
                   if(landingtrue == 'yes') { lpage.landingRequest();}
                } else {
                    $('.change-icon-img').show();
                    $('.zip-loader-img').hide();
                    $('#selectedZipCode').prop('disabled', false);
...
var validation = {
    digitValidate: function(digit_counts, field_id, err_msg_span, err_msg) {
        ...
        if (ValidateField.length != digit_counts) {
            ...
            // lpage.loaderHide('.loader'); 
            // if (localCache.get('orevtype') == 'cpo') {
            //     lpage.landingRequest();
            // }
        }
    }
