...
use Fcaore\Databucket\Exceptions\ConditionNotAllowedException;
use Fcaore\Databucket\Exceptions\InvalidJsonException;
use Fcaore\Databucket\Exceptions\InvalidNodeException;
use Fcaore\Databucket\Exceptions\NullValueException;
     ...
     */
      ...
      if (!is_null($jsonFile)) {
        ...
        */
		 ...
		 $this->possibleDuplicates = config('ore.etl.possible_duplicates');
    }
...
public function getVinInfo($params_vin, $params_make, $params_vechType,$params_year,$params_model){ return Databucket::getVinFromTrait($params_vin, $params_make, $params_vechType,$params_year,$params_model);}
    /**
     ...
     * @throws ConditionNotAllowedException
     */
    public function limit($start=0,$limit, $object = false)
    {
        $this->prepare();
        ...
        if (!is_integer($limit) || $total_elm < $idx || $limit == 0 || !is_array($this->_map)) {
            return null;
        }

        if ($limit > 0) {
            $result = array_slice($data, $start, $limit, true);
        } else {
            $result = $data[$this->count() + $limit];
        }
	...
	*/
	...
	public function sortByMiles($dealerOrder){
		...
		$keys = array_flip($dealerOrder);
 
		usort($this->_map, function($a, $b) use($keys)
		{
            if(count($keys)==0){                
                return false;
            }
			return $keys[$a['dealer_code']] - $keys[$b['dealer_code']];
		});
