...
<html class="no-js" lang="">
	...
	<meta charset="utf-8">
        ...
        <title> Search New Inventory - DriveFCA |@yield('title')</title>
        <link rel="icon" href=""  />
		
	<meta name="keywords" content="" /> 
    <meta name="description" content="The Mechanics of Emotions. The vehicles are crafted for performance.."/> 
    <meta name="pageName" content="brand-specific" />  
...
@endif
    <input type="hidden" id="APP_URL" value="@php echo env('APP_URL') @endphp" /> 
		...
		@include('partials.brand_header',['page' => 'landing', 'make' => $params_make]) 
            @yield('content')
        @include('partials.footer',['make' => $params_make])    
		 
		@if(env('APP_ENV') == 'local' || env('APP_ENV') == 'dev' || env('APP_ENV') == 'test' || env('APP_ENV') == 'training') 
			<script type="text/javascript" src="{{ asset('js/BrandLandingScript.js') }}"></script>  
		    <script type="text/javascript" src="{{ asset('js/merkle.js') }}"></script>
		@else
			<script type="text/javascript" src="{{ secure_asset('js/BrandLandingScript.js') }}"></script>  
