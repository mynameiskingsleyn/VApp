...
use App\Model\Discountfiltergroup;

class CalculatorController extends Controller
{	
	use CacheTrait;
	protected $modelDiscount,$modelDiscountFinance,$modelVinDiscount;
	...
	private $explore_inarray 			 = ['Loyalty','Lease Loyalty','Military','Automobility','Lease Conquest','Conquest'];		 
	...
	private $request_make_name,$request_dealer_code;
	/*
	* return array
	*/
	private $payment_calculator = array();
	
	public function __construct(Discount $modelDiscount,Vindiscount $modelVinDiscount,Financediscount $modelDiscountFinance,Filterdiscount $modelFilterDiscount)
    {  
		//\Log::info(' ********* ENVIRONMENT *****************');
		//\Log::info(env('APP_ENV'));
		date_default_timezone_set('Etc/GMT');
		 
		 $this->Dlrmgdiscount			 = $modelDiscount;
		...
		$this->payment_calculator['StatusCode'] = 1000;
		$this->partner_id = '';
		...
		$this->incentiveBonus_disclaimer = array(
			...
			'29HLF' => 'Offer is available on the purchase or lease of a new 2020 Pacifica or Pacifica Hybrid. Customer responsible for $200 administration fee. Must take retail delivery by 4/30/2020. See dealer for details.'
			);
    }
	...
	public function paymentcalc(Request $request){
		return $this->PaymentCalculator($request);
	}
	...
	public function PaymentCalculator($request){
		...
		$filters = array();
		$filters['dealer_code'] = $filter_info['DealerCode'];
		$filters['makeCode'] = isset($filter_info['makeCode']) ? $filter_info['makeCode'] : 'Y';
		$filters['make'] = $this->getMakeAbbrevationName($filters['makeCode']);
		$filters['year'] = isset($filter_info['modelyear']) ? $filter_info['modelyear'] : NULL;
		$filters['model'] = isset($filter_info['model']) ? $filter_info['model'] : NULL;
		$filters['trim_desc'] = isset($filter_info['trim']) ? $filter_info['trim'] : NULL;
		$filters['vin'] = $vin;
		\Log::info('==== VIN Info ====');
		\Log::info($filters);
		...
		$this->request_make_name = strtolower($filters['make']);
		$this->request_dealer_code = $filters['dealer_code'];

		if('alfa romeo' == $this->request_make_name){
			\config()->set('database.default', 'mysql');
			\Log::info('Connected DBHOST::'.\DB::connection()->getConfig("host"));
			$this->defaultFinanceType = 'AllyFinancialInc';
			$this->salesCodesArray = [];
			$this->payment_calculator['salesCodesArray']    =   $this->salesCodesArray;
		}else{
			\config()->set('database.default', 'mysql2');
			$this->defaultFinanceType = 'ChryslerCapital';
			$this->salesCodesArray = ['CCAP', 'diesel', 'engine', 'transmission', 'package', 'v6', 'v8', 'hemi'];
			$this->payment_calculator['salesCodesArray']    =   $this->salesCodesArray;
		}
		\Log::info('Connected DBHOST::'.\DB::connection()->getConfig("host"));
		...
		*/
		\Log::info(' ******** Routeone Environment | Calculator dealership id | Zipcode | Accessid ******* '); 
		if('alfa romeo' != $this->request_make_name){
			if(env('APP_ENV') == 'local' || env('APP_ENV') == 'dev' || env('APP_ENV') == 'test'){
				\Log::info(' == Stage Env === ');
				$isAlternateDealerCode = $this->get_mdoca_alternate_dealer_code($filters['dealer_code']);
		...
		if('alfa romeo' != $this->request_make_name){
			...
			if(config('ore.calc.incentivesBonusCash')){
				$this->incentivesBonusCash($vin,$filters['make'],$filters['dealer_code']);
			}  
		}
		...
		//   \Log::info("Downpayment Calculation");
		//    \Log::info($down);
		...
		if($this->transactionDiscountType=='finance'){
				...
				$f_tier = $this->isCacheHMGet($fallbackCalcTier, 'value'); 
      ...
      $NonCashCustomerQuoteKey = $this->RouteoneCalculator($jsonArray,config('ore.calc.'.$transactionType));  
		...
		if($this->request_term != '' && $this->transactionDiscountType=='finance'){
			if(count($NonCashCustomerQuoteKey['result']) > 1){
				...
				$intersect_apr = array_intersect_key($NonCashCustomerQuoteKey['result'], $minapr_payload);
				$NonCashCustomerQuoteKey['result'] = $intersect_apr;
			}
		}

		//$allterms = array_column($NonCashCustomerQuoteKeyResults, 'apr', 'term');
		//\Log::info($allterms);
	   ...
	   */
	   if(count($NonCashCustomerQuoteKey['result']) == 0 && $this->transactionDiscountType=='finance'){ 
		   ...
		   if($transactionType=='lease') { $jsonArray = $this->ArrayBuilderLease($vin, $transactionType, $zipcode, $down, $tradein, $term, $AllIncentiveIds, $mileage);
			   ...
			   $this->finance_tier = '1';
			   $this->isCacheHMSet($fallbackCalcTier, 'value', $this->finance_tier); 	
			   $jsonArray = $this->ArrayBuilderFinance($vin, $transactionType, $zipcode, $down, $tradein, $term, $AllIncentiveIds);
		...
		\Log::info($NonCashCustomerQuoteKey);
		 ...
		 if(!empty($NonCashCustomerQuoteKey['result']) == 0){
		...
		$this->payment_calculator['c_dealer_disc'] = $c_dealer_disc;
		...
		$toCacheCalculator['downpayment'] = $down;
		...
		$this->isCacheHMSet('user:experience:sessionid:'.Session::getId(), 'calculator',json_encode($toCacheCalculator));
		/*************** LEADS END **************/ 
         return $this->payment_calculator;
	}
	...
	*/
	...
	function CalculatorIncentives($vin, $transactionType){
        ...
        if(!$this->isCacheExists($cacheName)){  
            #Rebates Builder 
            ...
            $this->isCacheHMSet($cacheName, 'response',json_encode($response));  
            $this->isCacheHMSet($cacheName, 'vin',$vin);  
            $this->isCacheHMSet($cacheName, 'transactionType',$transactionType); 
            $this->isCacheHMSet($cacheName, 'incentiveIds',json_encode($this->man_incentives_id)); 
			$this->isCacheHMSet($cacheName, 'incentiveNames',json_encode($this->man_incentives_name));
			$this->isCacheHMSet($cacheName, 'man_incentives_sin_name',json_encode($this->man_incentives_sin_name));
			$this->isCacheHMSet($cacheName, 'man_incentives_disclaimer_name',json_encode($this->man_incentives_disclaimer_name));
			$this->isCacheHMSet($cacheName, 'man_incentives_exp_name',json_encode($this->man_incentives_exp_name));			
            $this->isCacheHMSet($cacheName, 'incentiveAmount',$this->incentive_amount); 
         }else{        
             $response = $this->isCacheHMGet($cacheName, 'response'); 
             $this->ExploreDetails(json_decode($response[0], true)); 
			 
			list($e_1) = $this->isCacheHMGet($cacheName, 'incentiveIds');
			list($e_2) = $this->isCacheHMGet($cacheName, 'incentiveNames');
			list($e_3) = $this->isCacheHMGet($cacheName, 'man_incentives_sin_name');
			list($e_4) = $this->isCacheHMGet($cacheName, 'man_incentives_disclaimer_name');
			list($e_5) = $this->isCacheHMGet($cacheName, 'man_incentives_exp_name');
            ...
            $encode_incentive_amount          	= $this->isCacheHMGet($cacheName, 'incentiveAmount');
	...
	function incentivesBonusCash($vin,$make,$dealer_code){
		try{
			...
			$cacheName = 'calc:incentives:bonuscash:'.$make.':'.$dealer_code;
				...
				if($this->hexists($cacheName, 'data')){
						$calc_allocation = $this->isCacheHMGet($cacheName, 'data'); 
							if(count($calc_allocation) > 0){
								 $list_incentives_bonuscash = json_decode($calc_allocation[0], true);
							}  
						
					}else{
							$list_incentives_json = $this->sqlIncentivesBonusCash($make,$dealer_code); 
							$list_incentives_bonuscash = $list_incentives_json->toArray(); 
							
							$this->isCacheHMSet($cacheName, 'data',$list_incentives_json); 
					} 
	...
	function verifyPaymentMethodAllocation($VinNumber, $transactionType){
		...
		try{
			...
			if($this->isCacheExists($VinNumber) ){ 
				 $vin_info_make = $this->isCacheHMGet($VinNumber, 'make');
				 $vin_info_model = $this->isCacheHMGet($VinNumber, 'model');
				 $vin_info_trim_desc = $this->isCacheHMGet($VinNumber, 'trim_desc'); 
				 $list_calc_array = [];
				  
					if($this->hexists('calc:allocation', 'data')){
						$calc_allocation = $this->isCacheHMGet('calc:allocation', 'data'); 
							if(count($calc_allocation) > 0){
								 $list_calc_array = json_decode($calc_allocation[0], true);
							}  
						
					}else{
							$list_calc_json = $this->sqlPaymentMethodAllocation();
							$this->isCacheHMSet('calc:allocation', 'data',$list_calc_json); 
							 $list_calc_array = $list_calc_json->toArray(); 
					} 
				  
				 
				 // \Log::info($vin_info_model[0]);
	...
	function dealerAutomatedCalculation($VinNumber, $transcation){
		try{
			...
			if($this->isCacheExists($VinNumber) ){ 
				 $vin_info['make'] = $this->isCacheHMGet($VinNumber, 'make');
				 $vin_info['model'] = $this->isCacheHMGet($VinNumber, 'model');
				 $vin_info['trim_desc'] = $this->isCacheHMGet($VinNumber, 'trim_desc'); 
				 $vin_info['year'] = $this->isCacheHMGet($VinNumber, 'year'); 
				 $vin_info['dealer_code'] = $this->isCacheHMGet($VinNumber, 'dealer_code');
				 if($this->transactionDiscountType == 'cash'){
					$financeoption =  3;
				}else if($this->transactionDiscountType == 'finance'){
					$financeoption =  2;
				}else{
					$financeoption =  1;
				}
				  $result = $this->SqlFiltergroups($vin_info,$financeoption); 
			}
		}catch(\Exception $e){
     ...
     * @return void
     */
	function dealerDiscountCalculationforVin($VinNumber, $price){
		$discountAmount = 0;
		...
		//$now = date('Y-m-d');
		 /* $list_discount = $this->SqlDlrmgdiscount::where('vin',$VinNumber)->where('bulk_discount','0')->where('start_date', '<=', $now)->where('end_date', '>=', $now)->select('id as discount_id','uuid','discount_name as name_of_discount','flat_rate','percent_offer','start_date as discount_start_date','end_date as discount_end_date','discount_saved as saved_discount')->get(); */ 
		 //$list_discount = $this->SqlDlrmgdiscount($VinNumber);
		if($this->transactionDiscountType == 'cash'){
			$financeoption =  3;
		}else if($this->transactionDiscountType == 'finance'){
			$financeoption =  2;
		...
		$list_discount = $this->SqlFiltergroups($this->vin_info, $financeoption);
				...
				if($list_discount_array_value['flat_rate'] == null || empty($list_discount_array_value['flat_rate']) || 
					$list_discount_array_value['flat_rate']==0 ){ 
						//if($list_discount_array_value['saved_discount'] == 1){
							...
							$list_discount_html .= '<h4 class="dealer-discount"><img alt="incentive-icon" src="/img/incentive-icon-5.png" width="20" height="20"><span class="">'.$list_discount_array_value['name_of_discount'].'</span><span><b>'.$list_discount_array_value['percent_offer'].'%</b></span></h4>';
						...
						//if($list_discount_array_value['saved_discount'] == 1){ 
							...
							$list_discount_html .= '<h4 class="dealer-discount"><img alt="incentive-icon" src="/img/incentive-icon-5.png" width="20" height="20"><span class="">'.$list_discount_array_value['name_of_discount'].'</span><span><b>$'.$list_discount_array_value['flat_rate'].'</b></span></h4>';
	...
	public function downCalculation($down){
		...
		if(config('ore.calc.dealer_discount')){
				...
				$this->dealerDiscountCalculation($vin, $down);
    ...
    public function JsonVehicleBuild($vin)
    {
		...
		\Log::info(" ========== JsonVehicleBuild ========== "); 
		\Log::info($vin); 
		# Validate Cache Available or Not.
		if(!$this->isCacheExists($vin)){ 		
			$data = \App\Model\Vehicle::where(['vin' => $vin])->first();			
		}else{
			$data = $this->isCacheGetAll($vin); 
		}
		...
		if(strtolower($objVehicle->vehicle_type) == 'new'){
			$sellingPrice = (int)$objVehicle->msrp;
		}else{
			$sellingPrice = (int)$objVehicle->internetPrice;
		}
		
		$this->payment_calculator['msrp_results'] = $sellingPrice; 

		\Log::info(" ========== MSRP CALCULATION BEGIN ========== "); 
		...
		if(config('ore.calc.dealer_discount')){
			    $this->payment_calculator['dlrDiscAmount'] = 0; 
				$this->dealerDiscountCalculationforVin($vin, $sellingPrice);
		...
		// if(config('ore.calc.incentives_deducted_in_msrp')){
		// 	$sellingPrice -= $this->incentive_amount;
	 	// 	$sellingPrice -= $this->explore_amount;
		// }  
		...
		if (config('ore.calc.additioanal_offer')) {
			if ($this->request_transactionType == 'lease') {
				$sellingPrice -= $this->request_lease_additional_disc;
				$this->payment_calculator['additional_discount'] = $this->request_lease_additional_disc;
			}
			if ($this->request_transactionType == 'finance') {

				$sellingPrice -= $this->request_finance_additional_disc;
            ...
            if(!empty($includevins) || !empty($excludevins)){
	            if(!$vinFoundFlag){
	            	...
	            	if(0 < count($result)){
	            		...
	            		//$response = $this->addDiscountofnewlyfoundVin($dealer_code,$filter_group[0]['id'],$financeoption,$vin,$discounts);
	            		return $result;            	   				
	    			}            	
	            }
	        }
	...
	public function get_mdoca_alternate_dealer_code($dealer_code){
    	...
    	if($this->hexists('calc:mdoca:alternatedealers', 'data')){
			$mdoca_alternate_dealers = $this->isCacheHMGet('calc:mdoca:alternatedealers', 'data'); 
			if(count($mdoca_alternate_dealers) > 0){
				 $mdoca_alternate_dealers_list = json_decode($mdoca_alternate_dealers[0], true);
			}
		}else{
			$mdoca_alternate_dealers = $this->sqlMdocaAlternateDealers();
			$this->isCacheHMSet('calc:mdoca:alternatedealers', 'data',$mdoca_alternate_dealers); 
	...
	*/
	public function sqlIncentivesBonusCash($make,$dealer_code){
		...
		$list_incentive = \DB::table('fca_ore_incentives_bonus_cash AS ibc')
					->where(['ibc.dealer_code' => $dealer_code])
	...
	public function SqlDlrmgdiscount($VinNumber){
		$now = date('Y-m-d');
		if($this->transactionDiscountType == 'cash'){
			$finance_option =  3;
		}else if($this->transactionDiscountType == 'finance'){
		...
		//\App\Model\Dlrmgdiscount::join('')
		return \DB::table('discounts as d')->join('vindiscounts as v','v.discount_id','=','d.id')->join('financediscounts as f','f.discount_id','=','d.id')->where(['v.vin' => $VinNumber,'f.finance_option' => $finance_option])->where('d.start_date', '<=', $now)->where('d.end_date', '>=', $now)->select('d.id as discount_id','d.uuid','d.discount_name as name_of_discount','d.flat_rate','d.percent_offer','d.start_date as discount_start_date','d.end_date as discount_end_date','d.discount_saved as saved_discount','f.finance_option as payment_mode')->get();
	}
}
